...
def getFloat32BE (bytes : ByteArray) (offset : Nat) : Float :=
  let getUInt32BE (bytes : ByteArray) (offset : Nat) : UInt32 :=
    ...
    (b1 <<< 24) ||| (b2 <<< 16) ||| (b3 <<< 8) ||| b4
  Float32.toFloat ∘ Float32.ofBits $ getUInt32BE bytes offset


def getUInt32 (i j k l : UInt8) : UInt32 :=
  (i.toUInt32 <<< 24) + (j.toUInt32 <<< 16) + (k.toUInt32 <<< 8) + l.toUInt32
